...
# المنهج الأكاديمي المكتمل (CEFR Framework)
FULL_SYLLABUS = {
    "A1": {
        "name": "Beginner (A1)",
        "lessons": {
            1: {"title": "Greetings", "focus": "Verb to be, basic pronouns"},
            2: {"title": "Routine", "focus": "Present simple, clock times"},
...
def init_db():
    with sqlite3.connect('academy.db') as conn:
        ...
        # تحديثات قاعدة البيانات التراكمية للحفاظ على التوافق
...
<body>
    ...
    <div id="gdprModal" class="modal">
        <div class="modal-content">
            <h2 style="color: #2c3e50;">🔒 سياسة الخصوصية والموافقة (GDPR)</h2>
            <p style="font-size: 14px; line-height: 1.6;">بموجب المعايير الأكاديمية ومعايير حماية البيانات، يرجى العلم بأننا نقوم بمعالجة صوتك ونصوصك لتحسين تجربتك التعليمية (دون تخزين دائم للملفات الصوتية). يرجى تقديم موافقتك الصريحة للاستمرار.</p>
        ...
        <div class="modal-content">
            ...
            <div id="parentReportArea" style="display:none;">
                <div class="parent-report">
                    <h3>📝 التقييم السلوكي والأكاديمي للطالب</h3>
                    <div id="aiParentSummary">⏳ جاري تحليل بيانات الطالب ومصفوفات CEFR...</div>
                </div>
    ...
    <div id="academicModal" class="modal">
        <div class="modal-content" style="line-height: 1.8;">
            ...
            <ul style="padding-right: 20px;">
                ...
                <li><b>المسار التعليمي الموجه:</b> <button onclick="sendMsg('Start the next syllabus lesson', true); closeModal('academicModal');" style="background:#8e44ad; color:white; border:none; border-radius:5px; cursor:pointer; padding:3px 8px; font-size:12px;">بدء الدرس المنهجي الحالي</button></li>
                <li><b>الإنجازات والشهادات:</b> اجمع 1500 XP واجتز الاختبارات.</li>
            </ul>
        </div>
    </div>
    ...
    <div style="max-width: 900px; margin: 0 auto; display: flex; justify-content: space-between; align-items: center;">
        <h2>Smart Academy 🎓 <span style="font-size:12px; color:#bdc3c7;" title="Enterprise Complete Monolithic Build">v1.3.1-Core</span></h2>
        <div style="font-size: 14px; font-weight: bold; color: #8e44ad;">مرحباً بك {{ username }}! | رصيد XP: <span id="xpDisplay">0</span></div>
    </div>
    
    <div class="progress-container" title="نسبة تقدمك في مستوى CEFR الحالي"><div id="mainProgress" class="progress-bar"></div></div>
    ...
    <script>
        ...
        function downloadFile(fileType) { window.location.href = '/download/' + fileType; closeModal('downloadsModal'); }
        function upgradeAlert() { alert("💎 هذا الملف مخصص لاشتراكات Pro و VIP فقط."); closeModal('downloadsModal'); }
        ...
        function applySettings() { 
            ...
            if (document.getElementById("mode").value === "child") { root.style.setProperty('--ai-bg', "#ffebef"); }
        }
        ...
        async function sendMsg(overrideMsg = null, isHidden = false) { 
            ...
            try { 
                ...
                if(data.workflow_state !== 'FREE_CHAT' && !isClassroomMode) { 
                    ...
                    } else if (data.workflow_state === 'LEVEL_EXAM') {
                        sBanner.innerHTML = `🏛️ امتحان مستوى CEFR الشامل (اضغط للإنهاء)`;
                        sBanner.style.background = "linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)";
...
def intro_audio():
    text = "مَرْحَبًا بِكَ فِي أَكَادِيمِيَّتِكَ الذَّكِيَّةِ لِتَعَلُّمِ اللُّغَةِ الْإِنْجِلِيزِيَّةِ."
    try:
...
def parent_dashboard():
    ...
    try:
        with sqlite3.connect('academy.db') as conn:
            cursor = conn.execute("SELECT parent_pin, last_quiz_score, fluency_score, grammar_score, vocab_score FROM users WHERE id = ?", (user_id,))
            row = cursor.fetchone()
            ...
            if action == 'verify': return jsonify({"success": True})
                ...
                prompt = f"""You are a professional educational assessor. Based on these recent messages from the student: {user_msgs}, and their accumulated CEFR scores (Fluency: {fluency}, Grammar: {grammar}, Vocabulary: {vocab}), provide a short summary for their parent IN ARABIC. Keep it encouraging and under 150 words."""
...
def chat():
    ...
    try:
        ...
        sys_msg, current_state, current_lesson_id, xp = WorkflowManager.process_state(user_id, user_msg, mode, custom_curriculum)
        ...
        return jsonify({ "english": eng, "arabic": ar, "keywords": parsed.get("keywords", ""), "summary": parsed.get("summary", ""), "audio": audio, "workflow_state": current_state, "current_lesson": current_lesson_id, "xp_points": xp })
...
def classroom_chat():
    ...
    try:
        ...
        1. You are teaching a VIRTUAL CLASSROOM. Be engaging.
        Respond ONLY in valid JSON: { "english": "...", "arabic": "...", "keywords": "", "summary": "", "scores": {} }"""
    ...
    except Exception as e: return jsonify({"error": "حدث خطأ غير متوقع."})
